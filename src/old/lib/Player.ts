...
class Player {
  constructor(ns) {
    ...
    this.software = {
      ...
      ssh: ns
        .ls('home')
        .some((file: string) => ['BruteSSH.exe'].includes(file)),
      ftp: ns
        .ls('home')
        .some((file: string) => ['FTPCrack.exe'].includes(file)),
      smtp: ns
        .ls('home')
        .some((file: string) => ['RelaySMTP.exe'].includes(file)),
      http: ns
        .ls('home')
        .some((file: string) => ['HTTPWorm.exe'].includes(file)),
      sql: ns
        .ls('home')
        .some((file: string) => ['SQLInject.exe'].includes(file)),
      formulas: ns
        .ls('home')
        .some((file: string) => ['Formulas.exe'].includes(file)),
    };
  }
}
